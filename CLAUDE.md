...
src/
  ...
  component_linecommand.js       # /p and /r commands + Alt+I action handler
                                 #   (handleLineAction, applyIndent). Hotkey
                                 #   wiring lives in component_window.js — see
                                 #   service_hotkeys.js for the dispatcher.
...
- **Hotkey routing (`service_hotkeys.js`)** — Window-scoped hotkeys (Alt+1..5, Alt+R, Alt+I, Alt+C today) are NOT bound by per-component document listeners. Each component calls `service_hotkeys_register(serviceWindow, "alt+1", cb)` against its `ServiceWindow` instance. A single document keydown listener dispatches each event to `ServiceWindow.activeWindow()`'s callback map. If no window is active, or the active window has no binding, the event is ignored. Combos are normalized to `"ctrl+alt+shift+<key>"` lowercase. There is no global (window-agnostic) registration path — add one only when a real use case appears. Browser-reserved combos (Ctrl+T/W/N, Ctrl+1..9, Ctrl+0) cannot be intercepted from a page; the dispatcher will silently never fire for them. Ctrl+Z/Y stay on the textarea (auto-prevented native action) but are gated on `service_hotkeys_is_active(editorServiceWindow)` so undo/redo only fires when the editor window is active.
- **`registerLineReaderHotkey()` was removed** in the hotkey-routing migration. All Alt-shortcuts that used to live there now register via `service_hotkeys_register` inside `createEditor()`.
